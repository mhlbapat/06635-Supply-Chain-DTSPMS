...
function route_swap(pickup_route, delivery_route, stack_assignment)
	...
	for i in 1:length_route-1
		...
		# Swap in delivery route
		temp_delivery_route[i], temp_delivery_route[i+1] = temp_delivery_route[i+1], temp_delivery_route[i]

		# Check if in the same stack
		if any([(temp_delivery_route[i] in temp_stack_assignment[j]) && (temp_delivery_route[i+1] in temp_stack_assignment[j]) for j in 1:nstacks])
			# Swap in stack assignment
			index_of_temp_i = [findfirst(!isempty, findall.(x->x==temp_delivery_route[i], temp_stack_assignment)), findall.(x->x==temp_delivery_route[i], temp_stack_assignment)[findfirst(!isempty, findall.(x->x==temp_delivery_route[i],temp_stack_assignment))][1]]
			index_of_temp_iplus1 = [findfirst(!isempty, findall.(x->x==temp_delivery_route[i+1], temp_stack_assignment)), findall.(x->x==temp_delivery_route[i+1], temp_stack_assignment)[findfirst(!isempty, findall.(x->x==temp_delivery_route[i+1], temp_stack_assignment))][1]]
			temp_stack_assignment[index_of_temp_i], temp_stack_assignment[index_of_temp_iplus1] = temp_stack_assignment[index_of_temp_iplus1], temp_stack_assignment[index_of_temp_i]

			# Swap in pickup route
			temp_pickup_route[findall(x->x==temp_delivery_route[i],temp_pickup_route)], temp_pickup_route[findall(x->x==temp_delivery_route[i+1],temp_pickup_route)] = temp_pickup_route[findall(x->x==temp_delivery_route[i+1],temp_pickup_route)], temp_pickup_route[findall(x->x==temp_delivery_route[i],temp_pickup_route)]
		end
